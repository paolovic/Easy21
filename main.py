...
def draw_action_value_function(optimal_value_function):
    ...
    x, y = np.meshgrid(x, y)
    z = optimal_value_function[x - 1, y - 1]
...
if __name__ == '__main__':
    ...
    action_value_function = game.monte_carlo_es(episodes=1000000)
    optimal_value_function = np.max(action_value_function, axis=2)
    draw_action_value_function(optimal_value_function=optimal_value_function)
    while True:
        game.play()
